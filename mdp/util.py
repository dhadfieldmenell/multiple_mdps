...
# @profile
def eta_solve(b, lu_solve, eta_list, transpose = False):
    """
    solve Ax = b with A = eta[-1]*eta[-2]*...*eta[1]*eta[0]*p*l*u
    if transpose specified, solves for A'x = b

    returns x

    if len(eta_list) > eta_max, A is computed explicitly and refactored into PLU
    if eta_max=-1, then it defaults to A.shape[0]/2.0

    eta_list is [(eta_vec, eta_inv_vec, k)] 
    eta = np.eye(N)
    eta[k,:] = eta_vec
    """
    b = np.copy(b)
    ...
    if transpose:
        ...
        for eta, eta_inv, k in eta_list:
            ...
            b += offset
        return b
    for eta, eta_inv, k in reversed(eta_list):
        b[k] = eta_inv.dot(b)
    return lu_solve(b)

#@profile
def eta_mult_vec_inplace(v, k, A):
    a_k = v.dot(A)
    A[k] = a_k

#@profile
def eta_mult_T(v, k, A, eta_T, inplace=False):
    result = eta_T.dot(A)
    return result
    # if inplace:
    #     a_k = v.dot(A)
...
class HeapPQ(object):
    """
    implements a priority q for a heap
    supports key updating

    adding an item that is already in the queue will update its ranking to the minimum of the two options

    operations are thread-safe
    """
    REMOVED = '<removed-task>'      # placeholder for a removed taks
    ...
    def add(self, task, priority=0):
        ...
        if task in self.entry_finder:
            if priority >= self.entry_finder[task][0]:
                # if this is larger than its value
                self.lock.release()
                return
            self._remove(task, get_lock=False)
    ...
    def remove(self, task):
        self._remove(task, get_lock = True)

    def _remove(self, task, get_lock = True):
        'Mark an existing task as REMOVED., should only be called internally'
        if get_lock:
            self.lock.acquire()
        if task in self.entry_finder:
            entry = self.entry_finder.pop(task)
            entry[-1] = HeapPQ.REMOVED
        if get_lock:
            self.lock.release()
